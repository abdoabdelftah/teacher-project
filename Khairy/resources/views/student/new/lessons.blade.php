                            ...
                            <div class="col-sm-6 col-lg-4">
                                <div class="card p-2 h-100 shadow-none border">
                                    <div class="rounded-2 text-center mb-3">
                                        <a href="/grade/{{ $grade->id }}/unit/{{ $unit->id }}/lessons"><img
                                                class="img-fluid"
                                                src="{{ $unit->image ? $unit->image : asset('admin/assets/img/pages/app-academy-tutor-1.png') }}"
                                                alt="tutor image 1" /></a>
                                    </div>
                                    <div class="card-body p-3 pt-2">
                                        <div class="d-flex justify-content-between align-items-center mb-3">
                                            <span class="badge rounded-pill bg-label-primary">عدد الدروس:
                                                {{ $unit->lessons->count() }}</span>

                                        </div>
                                        <a href="/grade/{{ $grade->id }}/unit/{{ $unit->id }}/lessons"
                                            class="h5">{{ $unit->name }}</a>
                                        <p class="mt-2">{{ $unit->description }}</p>

                                        <div class="progress rounded-pill mb-4" style="height: 8px">
                                            <div class="progress-bar w-{{ isset($percentages[$unit->id]) ? $percentages[$unit->id] : 0 }}"
                                                role="progressbar"
                                                aria-valuenow="{{ isset($percentages[$unit->id]) ? $percentages[$unit->id] : 0 }}"
                                        ...
                                        <div
                                            ...
                                            <a class="w-100 btn btn-outline-primary d-flex align-items-center"
                                                href="/grade/{{ $grade->id }}/unit/{{ $unit->id }}/lessons">
                                                <span class="me-1">الانتقال الى الدروس</span><i
